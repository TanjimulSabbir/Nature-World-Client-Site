        ...
        <div className="dropdown dropdown-start">
            ...
            <div tabIndex={0} onClick={() => setIsOpen(false)} className={`card dropdown-content text-black bg-green-100 shadow-2xl rounded-box w-64 ${isOpen ? "block" : "hidden"}`}>
                <div className="card-body">
                    <h2 className="card-title font-Fraunce font-bold">{AllBooking?.length} {AllBooking?.length > 1 && 'items' || 'item'}</h2>
                    <p className='font-lato Text-Primary'>Your Total Amount is ${TotalPrice?.toFixed(4)}.</p>
                    <p className='font-Montserrat mb-4'>You have Added to Cart {AllBooking?.length} Product{AllBooking?.length > 1 && 's'} and Total {TotalItems} Items.</p>
